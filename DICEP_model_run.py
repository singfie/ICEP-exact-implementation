...
def run_S_ICEP_model(m, dirname, vessel_source, is_docks_source,
                     iteration, completed_routes, gamma_value,
    ...
    route_details.to_csv(os.path.join(SOL_DIR, 'route_plan_scenario_GUROBI_gamma_' + str(gamma_value) + '_after_TRUE_DEMAND_REVEAL.csv'), index = False)
...
def main():
    ...
    parser.add_argument("-gamma", type = int, help="gamma value of the corresponding robust plan")
    ...
    if iteration > 0:
        previous_route_plan = pd.read_csv(os.path.join(SOL_DIR,
                                                       'route_plan_scenario_GUROBI_gamma_' + str(gamma_value) + '.csv'))
    ...
    m, completed_routes = DICEP_model_generator.main(vessel_source, vessel_pos_source,
